...
(deftest next-x-test
  (let [t #'lambdaisland.clj-diff.miller/next-x]
    (is (= (t 0  {             }) 0))
    (is (= (t 1  {0 2          }) 3))
    (is (= (t 2  {0 2, 1 5     }) 6))
...
(deftest snake-test
  (let [n (dec (count a1))
        m (dec (count b1))
        t (fn [k fp] (#'lambdaisland.clj-diff.miller/snake a1 b1 n m k fp))]
    ;; p = 0
    (is (= (t  0 {                               }) 2))
    (is (= (t  1 {            0 2                }) 5))
    (is (= (t  2 {            0 2, 1 5           }) 6))
  ...
  (let [n (dec (count a2))
        m (dec (count b2))
        t (fn [k fp] (#'lambdaisland.clj-diff.miller/snake a2 b2 n m k fp))]
    (is (= (t 0 {0 0}) 0))
    (is (= (t -2 {-1 0, 0 1}) 0))
    (is (= (t -1 {-2 0, -1 0, 0 1}) 1)))
  (let [n (dec (count a3))
        m (dec (count b3))
        t (fn [k fp] (#'lambdaisland.clj-diff.miller/snake a3 b3 n m k fp))]
    (is (= (t  0 {                    }) 2))
    (is (= (t -1 {            0 2     }) 2))
    (is (= (t  0 {      -1 2, 0 2     }) 3))
    (is (= (t -2 {      -1 2, 0 3     }) 2))
  ...
  (let [n (dec (count a4))
        m (dec (count b4))
        t (fn [k fp] (#'lambdaisland.clj-diff.miller/snake a4 b4 n m k fp))]
    (is (= (t   0 {         }) 2))
    (is (= (t  -1 {      0 2}) 2))
    (is (= (t   0 {-1 2, 0 2}) 3))
    (is (= (t  -2 {-1 2, 0 3}) 3))))
...
(deftest p-band-diagonals-test
  (let [t #'lambdaisland.clj-diff.miller/p-band-diagonals]
    (is (= (t 0 2) [0 1 2]))))

(deftest search-p-band-test
  (let [n (dec (count a1))
        m (dec (count b1))
        t (fn [p fp] (#'lambdaisland.clj-diff.miller/search-p-band a1 b1 n m 2 p fp))]
    (is (= (t 0 {})
           {0 2, 1 5, 2 6}))
    (is (= (t 1 {0 2, 1 5, 2 6})
           {-1 2, 0 6, 1 7, 2 8, 3 7}))
  ...
  (let [n (dec (count a2))
        m (dec (count b2))
        t (fn [p fp] (#'lambdaisland.clj-diff.miller/search-p-band a2 b2 n m 0 p fp))]
    (is (= (t 0 {})
           {0 0}))
    (is (= (t 1 {0 0})
           {-1 0, 0 1, 1 1}))
  ...
  (let [n (dec (count a3))
        m (dec (count b3))
        t (fn [p fp] (#'lambdaisland.clj-diff.miller/search-p-band a3 b3 n m 0 p fp))]
    (is (= (t 0 {})
           {0 2}))
    (is (= (t 1 {0 2})
           {-1 2, 0 6, 1 4})))
  (let [n (dec (count a4))
        m (dec (count b4))
        t (fn [p fp] (#'lambdaisland.clj-diff.miller/search-p-band a4 b4 n m 0 p fp))]
    (is (= (t 0 {})
           {0 2}))
    (is (= (t 1 {0 2})
           {-1 2, 0 3, 1 3}))
...
(deftest edit-dist-test
  (let [t (fn [p k] (#'lambdaisland.clj-diff.miller/edit-dist 0 p k))]
    (is (= (t 4 1)  7))
    (is (= (t 4 0)  8))
    (is (= (t 4 -1) 7))
    (is (= (t 2 0) 4))
    ...
    (is (= (t 1 1) 1))
    (is (= (t 0 0) 0)))
  (let [t (fn [p k] (#'lambdaisland.clj-diff.miller/edit-dist 2 p k))]
    (is (= (t 2 3) 5))
    (is (= (t 2 1) 5))
    (is (= (t 2 -1) 3))))

(deftest p-value-up-test
  (let [t (fn [p k] (#'lambdaisland.clj-diff.miller/p-value-up 0 p k))]
    (are [p k p*] (= (t p k) p*)
         1 -1 0
         1  0 1
         2  1 2
         ...
         2  0 2
         3  1 3))
  (let [t (fn [p k] (#'lambdaisland.clj-diff.miller/p-value-up 2 p k))]
    (are [p k p*] (= (t p k) p*)
         1 -1 0
         1  0 0
         1  1 0
...
(deftest p-value-left-test
  (let [t (fn [p k] (#'lambdaisland.clj-diff.miller/p-value-left 0 p k))]
    (are [p k p*] (= (t p k) p*)
         1  1 0
         1  0 1
         2 -1 2
         ...
         2  0 2
         3 -1 3))
  (let [t (fn [p k] (#'lambdaisland.clj-diff.miller/p-value-left 2 p k))]
    (are [p k p*] (= (t p k) p*)
         0  1 0
         1  0 1
         2 -1 2
...
(deftest look-up-test
  (let [t (fn [p x k] (#'lambdaisland.clj-diff.miller/look-up a1-graph 2 p x k))]
    (is (= (t 2 9 2) {:edit :insert :x 9 :p 2 :k 3 :d 5}))
    (is (nil? (t 2 9 3)))
    (is (= (t 1 5 0) {:edit :insert :x 5 :p 0 :k 1 :d 1}))
    (is (nil? (t 1 7 1))))
  (let [t (partial #'lambdaisland.clj-diff.miller/look-up a4-graph 0)]
    ...
    (is (nil? (t 2 3 -2))))
  (let [t (partial #'lambdaisland.clj-diff.miller/look-up a3-graph 0)]
    (is (= (t 1 4 0) {:edit :insert :x 4 :p 1 :k 1 :d 1})))
  (let [t (partial #'lambdaisland.clj-diff.miller/look-up a2-graph 0)]
    (is (= (t 3 0 -3) {:edit :insert :x 0 :p 2 :k -2 :d 2})))
  (let [t (partial #'lambdaisland.clj-diff.miller/look-up a5-graph 0)]
    (is (= (t 2 6 0) {:edit :insert :x 6 :p 2 :k 1 :d 3}))
    (is (= (t 2 6 1) {:edit :insert :x 6 :p 2 :k 2 :d 2}))))

(deftest look-left-test
  (let [t (partial #'lambdaisland.clj-diff.miller/look-left a1-graph 2)]
    (is (nil? (t 2 9 2)))
    (is (= (t 2 9 3) {:edit :delete :x 8 :p 1 :k 2 :d 4}))
    (is (= (t 1 8 2) {:edit :delete :x 7 :p 1 :k 1 :d 3}))
    (is (nil? (t 1 5 0))))
  (let [t (partial #'lambdaisland.clj-diff.miller/look-left a4-graph 0)]
    (is (= (t 4 9 0) {:edit :delete :x 8 :p 4 :k -1 :d 7}))
    (is (= (t 3 6 0) {:edit :delete :x 5 :p 3 :k -1 :d 5}))
    (is (= (t 3 5 -1) {:edit :delete :x 4 :p 3 :k -2 :d 4}))
    (is (nil? (t 3 3 -3))))
  (let [t (partial #'lambdaisland.clj-diff.miller/look-left a2-graph 0)]
    (is (nil? (t 3 0 -3)))))

(deftest next-edit-test
  (let [t (fn [p x k] (#'lambdaisland.clj-diff.miller/next-edit a1 b1 a1-graph 2 p x k))]
    (is (= (t 2 12 2)
  ...
  (let [t (fn [p x k] (#'lambdaisland.clj-diff.miller/next-edit a5 b5 a5-graph 0 p x k))]
    (is (= (t 2 6 0)
...
(deftest edits-test
  (let [t #'lambdaisland.clj-diff.miller/edits]
...
(deftest transpose-test
  (let [t #'lambdaisland.clj-diff.miller/transpose]
...
(deftest edits->script
  (let [t #'lambdaisland.clj-diff.miller/edits->script]
    (is (= (t b1 a1-edits identity)
           {:+ [[7 \e] [8 \c]] :- [2 5 6 7]}))
    (is (= (t a1 a1-edits #'lambdaisland.clj-diff.miller/transpose)
           {:+ [[1 \e] [3 \a \b \b]] :- [4 6]}))))
